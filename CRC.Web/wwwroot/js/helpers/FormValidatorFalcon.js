...
const _formValidator = selector => (function (selector = null) {
    let validate = function (selector = null) {
        ...
        let validated = [];
        let elements = $(selector).find(`:input.form-input`).not(`[type="hidden"], button`).toArray();
        elements.forEach(function (element) {
            if (!validated.includes(element.id) && !$(element).hasClass(`choices__input`)) {
        ...
        // For choices.js
        elements = $(selector).find(`select.form-input.choices__input`).toArray();
        elements.forEach(function (element) {
    ...
    let checkInput = function (element) {
        switch (element.type) {
            case `text`:
                return $(element).hasClass(`datetimepicker`) ? checkDate(element) : checkText(element);
                break;
            case `number`:
    ...
    let checkDate = function (element) {
        ...
        if (element.hasAttribute(`required`) && ($(element).val() === null || $(element).val().trim().length === 0)) {
            ...
            return false;
        } else if ((element.hasAttribute(`required`) || ($(element).val() !== null && $(element).val().trim().length !== 0)) && !moment($(element).val(), `YYYY-MM-DD`, true).isValid()) {
            $(element).parent().find(`.invalid-feedback`).text(`Invalid date format, please use YYYY-MM-DD!`);
            $(element).addClass(`is-invalid`);
            return false;
        }

(function (_dataTable, $, undefined) {
    ...
    * @return   {Void}
    */
    _dataTable.initialize = function ({
        resource = null,
        columnDefs = [],
        actionColumn = [`edit`],
        editAction = null,
    ...
    * @return   {Void}
    */
    _dataTable.initialize2 = function ({
        selector = null,
        resourceData = [],
        columnDefs = [],
        actionColumn = [`edit`],
    ...
    function populateColumnDefs(elementName, resourceData) {
        ...
        if (resourceData.length != 0) {
            ...
            for (let i = 0; i < resourceDataKey.length; i++) {
                $(`#${elementName}-table thead tr`).append(`<th>${resourceDataKey[i]}</th>`);
                $(`#${elementName}-table tfoot tr`).append(`<th>${resourceDataKey[i]}</th>`);
            ...
            if (actionColumn.includes(`edit`)) {
                if (editAction == null) {
                    ...
                    $(`table#${elementName}-table a.edit-entry`).on(`click`, function (event) {
                        ...
                        try { getRecord($(this).attr(`id`).split(elementName + `-`).pop()); }
                    ...
                    $(`table#${elementName}-table a.edit-entry`).on(`click`, function (event) {
                        ...
                        try { editAction($(this).attr(`id`).split(elementName + `-`).pop()); }
                        catch (error) { _toast.error(error.message); }
                    });
...
}(window._dataTable = window._dataTable || {}, jQuery));
